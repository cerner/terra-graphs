...
describe('Paired Result - Load', () => {
  ...
  it('internal subsets gets created correctly for each data point', () => {
    ...
    expect(
      data.internalValuesSubset.map((i) => constants.PAIR_ITEM_TYPES.map(
        (j) => i[j].x !== null
                        && i[j].y !== null
                        && i[j].label !== null
                        && i[j].shape
    ...
    expect(
      data.internalValuesSubset.every(
        (i) => i.onClick !== null && i.yAxis !== null && i.key === data.key,
      ),
    ).toBeTruthy();
  ...
  it('internal subsets gets created successfully for time series', () => {
    ...
    const data = graph.content[0].dataTarget;
    expect(
      data.internalValuesSubset.every((i) => constants.PAIR_ITEM_TYPES.map(
  ...
  it('defaults color to black when not provided', () => {
    ...
    const data = graph.content[0].dataTarget;
    expect(
      data.internalValuesSubset.every(() => constants.PAIR_ITEM_TYPES.map(
  ...
  it('defaults shapes to circle when not provided', () => {
    ...
    const data = graph.content[0].dataTarget;
    expect(
      data.internalValuesSubset.every(() => constants.PAIR_ITEM_TYPES.map((j) => j.shape === 'CIRCLE')),
    ).toBeTruthy();
  });

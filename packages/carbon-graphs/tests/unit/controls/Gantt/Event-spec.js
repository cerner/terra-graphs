...
describe('Gantt -> Track -> Event', () => {
  ...
  const getTranslate = (el) => getSVGAnimatedTransformList(d3.select(el).attr('transform')).translate;
  const xVal = new Date(2018, 4, 1, 6, 15).toISOString();
  const xValAlt = new Date(2018, 4, 1, 9, 15).toISOString();
  // eslint-disable-next-line no-shadow
  const loadData = (gantt) => gantt.loadContent(
  ...
  describe('On Load', () => {
    ...
    it('Renders selection data points correctly', () => {
      ...
      const defaultPlot = constants.DEFAULT_PLOT_SELECTION_SCALE_FOR_EVENTS;
    ...
    describe("Pass Through's", () => {
      ...
      describe('clickPassThrough - true', () => {
        const data = Object.assign(getAxes(axisJSON), {
          clickPassThrough: {
            events: true,
          },
        });

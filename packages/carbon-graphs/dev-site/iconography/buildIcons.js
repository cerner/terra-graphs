...
const iconParserLogger = (msg) => console.log(`----- Carbon SVG JSON parser: ${msg} -----`);
/**
...
const convertSVGToJSON = () => {
  ...
  }).then((output) => Object.keys(SHAPES).map((shape) => output.find((o) => shape in o)));
};

const writeDefinitionsIntoFile = (text) => fs.writeFileSync(
  outputPath,
...
const buildFunction = (shapes) => `${parserOptions.fileHeader()}
    ${shapes
  .map((i) => Object.keys(i).map(
    (s) => `${parserOptions.commentBlock(s)}

 ...
 */
const renderPopup = (axisType, event) => {
 ...
 */
export const loadLabelPopup = (axisLabel, axisType, event) => {
  removeOldPopup();
  const path = renderPopup(axisType, event);
  return path
    .append('g')
    .append('g')
...
const loadLabelShape = (shapeContainerPath, dataTarget) => {
  if (dataTarget.values.length > 0) {
    if (dataTarget.showShapes || dataTarget.showShapes === undefined) {
      return shapeContainerPath.append(() => new Shape(dataTarget.shape || SHAPES.CIRCLE).getShapeElement(
        getDefaultSVGProps({
    ...
    shapeContainerPath.append(() => new Shape(
 ...
 */
const getShapeContainerSize = (shapeContainerPath) => shapeContainerPath.selectAll('svg').size();
/**
 ...
 * @returns {Selection} d3 path for label shape container
 */
const translateYAxisLabelShapeContainer = (config, shapeContainerPath) => shapeContainerPath
  .transition()
 ...
 * @returns {Selection} d3 path for label shape container
 */
const translateY2AxisLabelShapeContainer = (config, shapeContainerPath) => shapeContainerPath
  .transition()
  .call(constants.d3Transition(config.settingsDictionary.transition))
  .attr(
    'transform',
 ...
 */
const translateAllLabelShapeItem = (shapeContainerPath, config) => shapeContainerPath
  .selectAll('svg')
  .transition()
  .call(constants.d3Transition(config.settingsDictionary.transition))

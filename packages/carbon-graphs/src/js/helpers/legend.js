 ...
 */
const legendDisplayStyle = (input) => (input.legendOptions
...
const loadLegendItem = (legendSVG, t, config, eventHandlers) => {
  ...
  const shouldForceDisableLegendItem = !!t.label.isDisabled || utils.isEmptyArray(t.values);
  ...
  if (!shouldForceDisableLegendItem && index > -1) {
    itemPath
      ...
      .on('mouseenter', () => eventHandlers.hoverHandler(t, constants.HOVER_EVENT.MOUSE_ENTER))
      .on('mouseleave', () => eventHandlers.hoverHandler(t, constants.HOVER_EVENT.MOUSE_EXIT));
  }
 ...
 */
const removeLegendItem = (legendSVG, dataTarget) => d3RemoveElement(legendSVG, `li[aria-describedby="${dataTarget.key}"]`);
/**
 ...
 * @returns {object} - d3 svg object
 */
const createLegend = (config, container) => container
  .append('ul')
  ...
  shownTargets
    ...
    .forEach((k) => {
      ...
      if (utils.notEmpty(additionalHandlers)) {
        additionalHandlers.forEach((fn) => fn(shownTargets, canvasSVG, key, hoverState, k));
      }
    });
};
/**
...
const loadPieLegendItem = (legendSVG, dataTarget, { hoverHandler }, config) => {
  ...
  itemPath.append(() => new Shape(getShapeForTarget(dataTarget)).getShapeElement(
    getDefaultSVGProps({
  ...
  itemPath
    .on('mouseenter', () => hoverHandler(dataTarget, constants.HOVER_EVENT.MOUSE_ENTER))
    .on('mouseleave', () => hoverHandler(dataTarget, constants.HOVER_EVENT.MOUSE_EXIT));
};
/**
...
const getDefaultLegendOptions = (graphConfig, dataTarget) => {
  ...
  legendOptions.showShape = dataTarget.showShapes !== false
...
const reflowLegend = (legendSVG, legend, graph, eventHandlers) => {
  ...
  const shouldForceDisableLegendItem = !!legend.label.isDisabled || utils.isEmptyArray(legend.values);
  const itemPath = legendSVG
  ...
  if (!shouldForceDisableLegendItem) {
    ...
    itemPath
      ...
      .on('mouseenter', () => eventHandlers.hoverHandler(
        legend,
        constants.HOVER_EVENT.MOUSE_ENTER,
      ))
      .on('mouseleave', () => eventHandlers.hoverHandler(
        legend,

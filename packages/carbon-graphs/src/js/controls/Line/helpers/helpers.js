 ...
 * @returns {object} d3 select object
 */
const translatePoints = (scale, canvasSVG, cls, config) => canvasSVG
...
const translateLineGraph = (scale, canvasSVG, config) => {
  canvasSVG
    .select(`.${styles.currentLinesGroup}`)
    .attr(
      'transform',
 ...
 * @returns {undefined} - returns nothing
 */
const drawDataLines = (scale, config, lineGroupSVG) => lineGroupSVG
  .append('g')
  ...
  .attr(
    'style',
    (value) => `stroke: ${getColorForTarget(value)}; stroke-dasharray: ${
...
const drawDataPoints = (scale, config, pointGroupPath, legendSVG) => {
  ...
  const renderSelectionPath = (path, value, index) => path.append(() => new Shape(
    getSVGObject(
      SHAPES.CIRCLE,
  ...
  const renderCriticalityPath = (path, value, index, cls) => path.append(() => new Shape(getShapeForTarget(value)).getShapeElement(
    getDefaultSVGProps({
...
const draw = (scale, config, canvasSVG, dataTarget, legendSVG) => {
  ...
  console.log("this is x value   "+getXAxisXPosition(config))

...
const { DEFAULT_HEIGHT } = constants;
 ...
 */
const getAxisInfoRowLabelHeight = (config) => (utils.isDefined(config.axisInfoRowLabelHeight)
 ...
 */
const translateCanvas = (config, canvasSVG) => canvasSVG
 ...
 * @returns {string} provided shape or a circle.
 */
const getColorForTarget = (dataPoint) => dataPoint.color || constants.DEFAULT_COLOR;
/**
...
const drawNoDataView = (config, svg) => {
  ...
  const noDataViewHeight = getYAxisHeight(config) / constants.NO_DATA_VIEW_PROPORTION;

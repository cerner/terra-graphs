 ...
 */
const isTrackLabelOutsideDomain = (scale, config, yVal, index) => index < 0
...
const getTransformYCoordinate = (scale, config, yVal) => {
  ...
  const getCurrentTrackHeightList = (trackList) => Object.keys(trackList).map((o) => trackList[o].trackHeight);
 ...
 */
const getTrackLabelTransformProperty = (scale, config) => (trackName) => `translate(${getTransformXCoordinate()}, ${getTransformYCoordinate(
  scale,
  config,
  trackName,
)})`;
 ...
 * @returns {object} d3 svg path
 */
const translateHorizontalGrid = (axis, config) => axis.y.tickSize(getXAxisWidth(config) * -1, 0, 0).tickFormat('');
 ...
 * @returns {object} d3 svg path
 */
const translateContentContainer = (config, canvasSVG) => canvasSVG
 ...
 * @returns {undefined} - returns nothing
 */
const translateLabelText = (scale, config, canvasSVG) => canvasSVG
 ...
 */
const transformPoint = (scale, config) => (value) => (scaleFactor) => `translate(${getTransformXCoordinate(
  scale,
  value.x,
 ...
 */
const translatePoints = (scale, config, trackPath, style) => trackPath.selectAll(style).each(function (d) {
 ...
 * @returns {undefined} - returns nothing
 */
const translateTaskBar = (scale, path, config) => path
  .transition()
  .call(constants.d3Transition(config.settingsDictionary.transition))
 ...
 * @returns {undefined} - returns nothing
 */
const translateActivityBar = (scale, path, config) => path
  .transition()
  .call(constants.d3Transition(config.settingsDictionary.transition))
  .attr('x', (val) => scale.x(val.startDate))
  .attr(
    'y',
    (val) => scale.y(val.y) + constants.DEFAULT_GANTT_ACTIVITY_PADDING.top,
  )
  .attr(
    'width',
    (val) => scale.x(val.endDate) - scale.x(val.startDate)
 ...
 */
const translateTasks = (scale, config, trackPath) => trackPath
  .selectAll(`g.${styles.task}`)
  .selectAll('rect')
  .each(function () {
    const path = d3.select(this);
    if (path.classed(styles.taskBarSelection)) {
...
const translateTrackSelector = (scale, config, trackPathSVG, trackConfig) => {
  ...
  ganttTrackSelectorGroupPath.select('rect').each(function () {
    const path = d3.select(this);
    const args = generatorArgs(config, scale, path, trackConfig);
    path.transition()
      .call(constants.d3Transition(config.settingsDictionary.transition))
      .attr('y', args.y)
 ...
 * @returns {Selection} - d3 Track selection
 */
const translateActivities = (scale, config, trackPath) => trackPath

...
const drawActivities = (
  ...
  const activityPath = activityGroupPath
    .selectAll('g')
    .data(
      activities.map((a) => processActivities(config, trackLabel, utils.deepClone(a))),
    );
  activityPath
    .enter()
    .append('g')
 ...
 */
const unloadActivities = (graphContext, trackPathSVG) => trackPathSVG.select(`g.${styles.activityGroup}`).remove();

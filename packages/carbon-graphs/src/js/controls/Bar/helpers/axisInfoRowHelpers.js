 ...
 */
const getTextLabelsXPosition = (attributeHelper, textLabelList, index) => (utils.isDefined(index) && index < textLabelList.length
 ...
 */
const getTextLabelsYPosition = (config) => (isXAxisOrientationTop(config.axis.x.orientation)
 ...
 */
const addTextLabelIcon = (axisInfoRow, config, textLabelList, uniqueKey) => axisInfoRow.select(`.${styles.axisInfoRowIcon}`).append(() => new Shape(getShapeForTarget(textLabelList.value)).getShapeElement(
  getDefaultSVGProps({
 ...
 */
const getTextLabel = (label, characterCount) => (shouldTruncateLabel(label, characterCount)
...
const createAxisInfoRowLabel = (
  ...
  const x = textLabelList;
  if (config.axis.x.type === AXIS_TYPE.TIME_SERIES) {
    x.x = utils.parseDateTime(x); // for time series
  }
 ...
 */
const removeAxisInfoRowLabels = (axisInfoRowSVG, key) => d3RemoveElement(
  axisInfoRowSVG,

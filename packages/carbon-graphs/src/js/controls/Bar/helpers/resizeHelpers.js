 ...
 */
const getBandwidthPaddingRatio = (content, shownTargets) => constants.DEFAULT_BAR_GRAPH_PADDING_ATTRIBUTES.OUTER_PADDING_RATIO
...
const setBarOffsets = (content, contentConfig, input, bandScale, config) => {
  const { shownTargets } = config;
  const group = contentConfig.filter(
    (d) => d.key === input.config.key

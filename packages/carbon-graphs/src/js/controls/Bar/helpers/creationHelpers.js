 ...
 */
const hasInvalidTicks = (xTicks, inputValues) => inputValues.some((value) => xTicks.indexOf(value.x) < 0);
 ...
 */
const hasInvalidTypes = (type, inputValues) => inputValues.some((value) => !isValidAxisType(value.x, type));
/**
 ...
 */
const hasMissingAxisInfoRowValue = (inputAxisInfoRow) => inputAxisInfoRow.some((axisInfoRow) => utils.isUndefined(axisInfoRow.value));
/**
 ...
 */
const hasSecondaryDisplay = (dataTarget) => dataTarget.axisInfoRow.some(
  ...
  return barGroupSVG
    ...
    .each(function (dataPoint, index) {
      ...
      if (utils.notEmpty(regionList)) {
        const regions = regionList.filter((r) => utils.isEqual(r.x, dataPoint.x));
      ...
      if (utils.notEmpty(axisInfoRowList)) {
        const textLabels = axisInfoRowList.filter((t) => utils.isEqual(t.x, dataPoint.x));
 ...
 */
const clear = (canvasSVG, key) => d3RemoveElement(canvasSVG, `g[aria-describedby="${key}"]`);

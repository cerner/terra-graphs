 ...
 */
export const bubbleScale = (dataTarget) => d3
  .scaleLinear()
...
export const generateColor = (dataTarget) => {
  const { lowerShade } = dataTarget.hue;
  const { upperShade } = dataTarget.hue;
  let radiusData;
  if (utils.isUndefined(dataTarget.weight)) {
    radiusData = dataTarget.values.map((element) => element.y);
  } else {
    radiusData = dataTarget.values.map((element) => bubbleScale(dataTarget)(element.weight));
  }

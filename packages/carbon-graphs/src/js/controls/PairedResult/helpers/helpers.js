 ...
 */
const translateLines = (scale, canvasSVG, config) => canvasSVG
 ...
 * @returns {object} - d3 select object
 */
const translatePoints = (scale, canvasSVG, config) => iterateOnPairType((type) => {
  canvasSVG
    .selectAll(
 ...
 */
const drawLine = (scale, config, boxPath) => boxPath.each(function (value, index) {
  const shouldCreateLine = (d) => d.high
      && d.low
...
const drawCriticalityPoints = (
  ...
  const renderPoint = (path, t, v, i, cls) => path.append(() => new Shape(getShapeForTarget(getValue(v, t))).getShapeElement(
    getDefaultSVGProps({
      svgClassNames: `${cls}`,
      ...
      a11yAttributes: {
        ...
        'aria-disabled': !utils.isFunction(v.onClick),
      },
    }),
  ));
  renderPoint(
  ...
  dataTarget.internalValuesSubset = dataTarget.values.map((value) => {
    ...
    iterateOnPairType((t) => {
      if (utils.isDefined(getValue(value, t))) {
        const currentValue = getValue(value, t);
        subset[t] = {
 ...
 * @returns {object} - d3 append object
 */
const showLine = (config, boxPath) => boxPath.each(function () {
  const shouldCreateLine = (d) => d.high
            && d.low
            && utils.hasValue(config.shownTargets, d.high.key)
            && utils.hasValue(config.shownTargets, d.low.key)
 ...
 */
const isRegionMappedToAllValues = (value, regionList) => Object.keys(value).every((v) => Object.prototype.hasOwnProperty.call(regionList, v));
/**

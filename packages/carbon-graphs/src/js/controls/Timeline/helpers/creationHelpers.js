 ...
 */
const determineHeight = (config) => DEFAULT_HEIGHT + config.padding.top - config.padding.bottom;
/**
 ...
 * @returns {number} Position for the axis
 */
const getXAxisYPosition = (config) => config.padding.top * 1.5 + config.padding.bottom;
 ...
 */
const getXAxisWidth = (config) => config.canvasWidth - config.padding.left - config.padding.right;
/**
 ...
 * @returns {number} Position for the label
 */
const getXAxisLabelYPosition = (config) => getXAxisYPosition(config)
 ...
 * @returns {object} d3 svg path
 */
const createTimelineContent = (config, canvasSVG) => canvasSVG
  .append('g')
 ...
 * @returns {object} d3 svg path
 */
const createTimelineContentGroup = (config, canvasSVG, contentConfig) => canvasSVG
  .append('g')
...
const dataPointActionHandler = (value, index, target) => {
  ...
  toggleDataPointSelection(target).call((selectedTarget) => value.onClick(
    () => {
...
const renderPointPath = (
  ...
  path.append(() => new Shape(getSVGObject(shapeStyle, objectScale)).getShapeElement(
    getDefaultSVGProps({
...
const createPoints = (scale, config, canvasSVG) => {
  ...
  const renderCriticalityPath = (path, value, index, cls, conf) => {
    let fillStyle;
    const objectScale = constants.DEFAULT_TIMELINE_SCALE;
    const hiddenConfig = document
      .querySelector(
    ...
    const graphProperties = { scale, config: conf, cls };
    renderPointPath(pathProperties, dataPointProperties, graphProperties);
  };
  canvasSVG
    .append('g')

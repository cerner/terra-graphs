 ...
 */
const getSelectedData = (canvasSVG, value, config) => canvasSVG
  .select(
...
const updateSelectionBars = (internalValueSubset, canvasSVG, config) => {
  ...
  selectionPath.each((dataPoint) => {
    const value = internalValueSubset.filter((v) => (config.axis.x.type === AXIS_TYPE.TIME_SERIES
...
const selectionAttributeHelper = (scale, bandScale, shownTargets, datum) => {
  ...
  return {
    ...
    height: () => (utils.notEmpty(shownDatum)
      ? bottomRange - topRange + 2 * PADDING
      : 0),
    width: () => (utils.notEmpty(shownDatum)
 ...
 */
const translateSelectBars = (scale, bandScale, canvasSVG, config) => canvasSVG.selectAll(`.${styles.taskBarSelection}`).each((dataPoint, i) => {

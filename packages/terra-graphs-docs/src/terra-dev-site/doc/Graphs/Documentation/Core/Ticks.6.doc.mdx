...
**Note:** If tick values are in between `[-0.5 < x < 0.5 and x != 0] or [-0.05 < x < 0.05 and x != 0]` and so on till `[-0.00005 < x < 0.00005 and x != 0]` which may leads to displaying multiple zero's on Y or Y2 axes, we set the formatting to be `-Math.floor(Math.log10(x))` decimal place.
...
| ------------------- | -------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| lowerStepTickValues | array    | []      | Lower-Tick-Values in the axis. Each tick will have an associated grid line. When lowerTickValues are present, values are disregarded. Lighter when compared to upperTickValues but darker than midpointTickValues.                                                       |
| midpointTickValues  | array    | []      | Mid-Tick-Values in the axis. Each tick will have an associated grid line. Lightest gray shade when compared to others. Uses the default coloring of values, if none of the lowerTickValues, midTickValues and upperTickValues are provided |

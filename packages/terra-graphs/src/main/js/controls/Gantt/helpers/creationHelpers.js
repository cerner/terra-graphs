 ...
 */
const determineHeight = (config) => (utils.isEmpty(config.axis.y.trackList)
  ? 0
 ...
 */
const updateTrackCount = (config, isLoad = false) => (isLoad ? config.axis.y.trackCount++ : config.axis.y.trackCount--);
 ...
 */
const formatTrackLabel = (label) => (shouldTruncateLabel(label) ? truncateLabel(label) : label);
/**
 ...
 */
const prepareYAxis = (scale, height) => d3
  .axisLeft(scale)
 ...
 */
const getXAxisYPosition = (config) => (config.padding.top + config.padding.bottom) * 2;
/**
 ...
 */
const getYAxisDomain = (trackList) => (utils.isEmpty(trackList)
  ? []
 ...
 */
const getYAxisRange = (trackList) => Object.keys(trackList)
  .map((o) => trackList[o].trackHeight)
  .map((h, index, arr) => (index > 0 ? h + arr.slice(0, index).reduce((a, b) => a + b, 0) : h));
/**
 ...
 */
const createContentContainer = (config, canvasSVG) => canvasSVG
  .append('rect')
 ...
 * @returns {object} d3 svg path
 */
const createTrackSelectionContainer = (config, canvasSVG) => canvasSVG.append('g').classed(styles.ganttTrackSelectorGroup, true);
/**
 ...
 * @returns {object} d3 svg path
 */
const createGanttContent = (config, canvasSVG) => canvasSVG
  .append('g')
 ...
 */
const createTrackContainer = (config, canvasSVG, trackConfig) => canvasSVG
 ...
 * @returns {Selection} - d3 svg path
 */
const removeTrackContainer = (canvasSVG, key) => canvasSVG.select(`g[aria-describedby="${key}"]`).remove();

...
const trackClickActionHandler = (path, trackConfig) => {
  ...
  toggleTrackSelection(path, trackConfig.key).call((d3TargetNode) => trackConfig.onClick(
    () => {
      const isSelected = d3TargetNode.attr('aria-selected') === 'true';
 ...
 */
export const unloadGanttTrackSelector = (graphContext, trackPath) => trackPath.select(`g.${styles.ganttTrackSelectorGroup}`).remove();
